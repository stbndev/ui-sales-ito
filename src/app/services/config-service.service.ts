...
export class ConfigService {
  ...
  Get(serviceName: String): Observable<ResponseNocheServices> {
    ...
    return this.http
      .get<ResponseNocheServices>(`${this._uriResources}${serviceName}`, {
        headers,
      })
      .pipe(map((rns) => rns));
  }
  ...
  Login(serviceName: String, data: ILogin): Observable<ResponseNocheServices> {
    return this.http
      .post<ResponseNocheServices>(`${this._uriResources}${serviceName}`, data)
      .pipe(
        map((res) => {
          if (res.flag === eCSTATUS.OK) {
            debugger;
            const tmpJWT = atob(res.data.split(".")[1]);
            const tmp = JSON.parse(JSON.parse(tmpJWT).data);
            const user: IUser = tmp;
            debugger;
            user.token = res.data;
  ...
  Make(
    ...
    tipo: any,
    data: any
  ): Observable<ResponseNocheServices> {
    ...
    switch (tipo) {
      case eTipos.POST:
        return this.http
          .post<ResponseNocheServices>(
            `${this._uriResources}${serviceName}`,
            data
          )
          .pipe(map((rns) => rns));
      ...
      case eTipos.DELETE:
        return this.http
          .delete<ResponseNocheServices>(`${this._uriResources}${serviceName}`)
          .pipe(map((rns) => rns));
  ...
  changeUserInfo(userInfo: any, pagetitle: string) {
    (userInfo.pagetitle = pagetitle), this.userSubject.next(userInfo);
  ...
  ProductsInCart(product: Productsmodel, remove?: boolean, clear?: boolean) {
    ...
    if (clear) {
      ...
      let found = this._listProductsInCart
        .getValue()
        .find((element) => element.idproducts == product.idproducts);
  ...
  changeListProductsDataAdd(item: IProducts) {
    let found = this._listproductsSource
      .getValue()
      .find((element) => element.idproducts == item.idproducts);
    if (found) {
      ...
      this._listproductsSource.next(
        this._listproductsSource.getValue().concat(item)
      );
    }
  }
  handleError(): void {}
  ...
  Upload(serviceName: String, data: any): Observable<any> {
    return this.http
      .post(`${this._uriResources}${serviceName}`, data)
